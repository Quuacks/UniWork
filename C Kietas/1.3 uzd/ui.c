...
int main() {
   ...
   printf("Ivedami skaiciai ir programa isveda skaicius didejancia tvarka bei kiek kartu jie pasikartoja \n");

   // ask to enter 10 numbers
   printf("iveskite %d skaitmenu tarp 0 ir %d \n", SIZE, FREQ_SIZE);
   ...
   while(counter < SIZE){
      ...
      if((scanf("%d", &num) == 1) && (getchar() == '\n')){
         if ((num >= 0) && (num <= FREQ_SIZE)){
            ...
            printf("Netinkama ivestis! Virsity masyvo reziai [0; %d]\n", FREQ_SIZE);
         }
      } else {
         // report invalid input, ask to enter whole non-negative number again
         printf("Netinkama ivestis! Iveskite neneigiama sveika skaiciu [0; %d] \n", FREQ_SIZE);
         while(getchar() != '\n')
            ;
      }
   }
